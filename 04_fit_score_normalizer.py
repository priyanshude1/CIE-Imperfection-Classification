...
REQUIRED_MANIFEST_COLS = ["split", "events_csv_path"]
SCORE_COL = "score"
...
def main() -> None:
    ...
    require_columns(df, REQUIRED_MANIFEST_COLS, "split manifest")
    ...
    if train_rows.empty:
        raise RuntimeError("No train rows found in manifest.")

    train_raw_list: List[np.ndarray] = []
    n_train_files = 0
    for p in train_rows["events_csv_path_path"].tolist():
    ...
    # Write artifacts
    normalizer_name = f"score_normalizer_seed{seed}.json" if seed is not None else "score_normalizer.json"
    ...
    payload = {
        ...
        "n_train_samples": int((df["split"] == "train").sum()),
        "notes": "Compute s_raw = log1p(score), then s_norm = (s_raw - mu)/sigma using train-fitted mu/sigma.",
    }
    normalizer_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    diag_name = f"score_normalizer_diagnostics_seed{seed}.csv" if seed is not None else "score_normalizer_diagnostics.csv"
